...
class ClientDatabase:
    ...
    def __init__(self, name):
        ...
        mapper(self.HistoryMessage, history_message)
    ...
    def save_message(self, contact, direction, message):
        ...
        messages = self.HistoryMessage(contact, direction, message)
        self.session.add(messages)
        self.session.commit()

    def get_ontacts(self):
    ...
    def get_history(self, contact):
        ...
        query = self.session.query(self.HistoryMessage).filter_by(contact=contact)

        return [(history.contact, history.direction, history.message, history.date)
                for history in query.all()]


if __name__ == '__main__':
    test_db = ClientDatabase('Nick')
    test_db.add_contact('Roy')
    test_db.add_contact('Tom')
    test_db.add_users(['Nick', 'Roy', 'Tom'])
    test_db.save_message('test2', 'out', f'тестовое сообщение {datetime.datetime.now()}!')
    print(test_db.get_contacts())
    print(test_db.get_users())
    print(test_db.check_user('Nick'))
    print(test_db.check_user('test'))
    print(sorted(test_db.get_history('test2')))
    test_db.del_contact('Roy')
    print(test_db.get_contacts())
